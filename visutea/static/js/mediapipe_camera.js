...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const hudGazeBar = document.getElementById("hudGazeBar");
    ...
    function updateUI(analysis, rppgResult) {
        ...
        if (gazeScore) gazeScore.textContent = au04Val;
        if (hudGazeBar) {
            hudGazeBar.style.width = `${au04Val}%`;
            hudGazeBar.style.backgroundColor = au04Val >= 60 ? "var(--accent-orange)" : "var(--accent-blue)";
        }
        if (gazeStatus) {
            gazeStatus.textContent = au04Val >= 60 ? "Franzido" : "Normal";
            gazeStatus.style.color = au04Val >= 60 ? "var(--accent-orange)" : "var(--accent-blue)";
        }
