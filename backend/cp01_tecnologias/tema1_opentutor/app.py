...
st.sidebar.markdown("Para rodar a IA, insira sua chave da API do Gemini.")
api_key_input = st.sidebar.text_input("Chave API (Gemini):", type="password")
...
with col2:
    ...
    if gerar:
        if not model:
            st.error("⚠️ Coloque a sua chave de API do Gemini na barra lateral esquerda antes de começar.")
        elif len(texto_aluno) < 50:
            ...
            with st.spinner("Analisando o texto e aplicando reengenharia pedagógica..."):
                try:
                    # Prompts Socráticos baseados na pesquisa acadêmica
                    ...
                    st.write("Verifique se a chave de API é válida e pertence ao Google Gemini.")
    else:
